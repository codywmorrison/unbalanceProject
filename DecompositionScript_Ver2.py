...
##        endT2 = '2023-6-30 00:00:00'
        ...
        dataMatrix = [[VdataCa,VdataBa,VdataAa],[VdataCb,VdataBb,VdataAb],[CdataCa,CdataBa,CdataAa],[CdataCb,CdataBb,CdataAb]] # i did swap these
        print(dataMatrix)
...
##                time = '2022-2023'
##               
##            plt.title('{b}: {c} Sample Power Profile with SIB and RIB Components - {a}'.format(a=titleA,b=txrName,c=time))
##
##            xlim1 = 1000 #~1 week = 1000 points
##            plt.xlim([xlim1,xlim1+250]) 
##            
...
##            plt.savefig('Decomp/A. Decomp Images/{a}.png'.format(a=txrName+' '+time+' Sample Profile',b=txrName),dpi=1200)
##                       
##            plt.close()
            #plt.show()
...
decompList = np.zeros((1,10),dtype=object)
decompList[0] = ['No. in DF23','TXR Name','Year','Type','I Max (A)','I Min (A)','% Fit','DPIB Med. (%)','SIB Med. (A)','RIB Med. (A)']
    ...
    try:
        ...
        for g in range(2):
            ...
            if A != []:
                ...
                Pa = np.array(D) #3*np.array(A)*
                Pb = np.array(E)      # <--- 3*Vph*Iph = S (apparent power)
                Pc = np.array(F) #3*np.array(C)* 3*np.array(B)*
                print(Pa)
                print(Pb)
                print(Pc)
                print(txrName)
                print(a)
                print(np.median(Pa),np.median(Pb),np.median(Pc))
            ...
            arrDecompList = np.array([i,txrName,time,scenario,maxP,minP,pFit,DPIB,SIB,RIB]).astype(str)
            #print(arrDecompList)
...
nameDF = 'V9 YearSet Current Component Decomposition_{a} to {b}.csv'.format(a=recordI,b=i)
